...
export const ReviewQueueView: React.FC<ReviewQueueViewProps> = ({
  ...
  return (
    <div className="h-full overflow-y-auto p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 text-white select-none">
      ...
      ───────────────────────────────────────────────────────────── */}
      {exportNotice && (
        <div className="bg-[#101726] border border-sky-500/40 text-sky-300 px-5 py-3 rounded-2xl flex items-center justify-between text-xs font-semibold shadow-lg animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-1.5 p-1 rounded-xl bg-[#0c121e] border border-sky-500/20 text-xs">
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg font-semibold transition-all cursor-pointer flex items-center gap-2 ${
              activeTab === 'rooms_table'
                ? 'bg-sky-600 text-white font-extrabold shadow-[0_2px_12px_rgba(2,132,199,0.35)] border border-sky-400/30'
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <div className="relative w-48 sm:w-56">
              ...
              <input
                ...
                className="w-full bg-[#0c121e] border border-sky-500/20 rounded-xl pl-8 pr-3 py-1.5 text-xs text-white placeholder-white/40 focus:outline-none focus:border-sky-400 focus:ring-1 focus:ring-sky-500/30"
              />
            </div>
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-colors cursor-pointer ${
                  roomFilter === 'all'
                    ...
                    : 'text-white/50 hover:text-white bg-[#0c121e] border border-white/10'
                }`}
              >
                Tất cả
              </button>
      ...
      {activeTab === 'rooms_table' && (
        <div className="bg-[#0e1424] border border-white/10 rounded-2xl overflow-hidden shadow-xl">
          ...
          <div className="px-5 py-3.5 bg-[#111728] border-b border-sky-500/20 flex items-center justify-between text-xs font-semibold">
            <div className="flex items-center gap-2 text-white/90">
              <Layers className="w-4 h-4 text-sky-400" />
              <span>Chi tiết bóc tách hình học các không gian ({filteredRooms.length} phòng)</span>
            </div>
            <div className="text-sky-200/60 text-[11px] font-mono">
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#0a0f1d] border-b border-white/10 text-white/50 text-[11px] uppercase tracking-wider font-semibold">
                  ...
                  <th className="py-3 px-3 text-right text-rose-400">Trừ cửa (m²)</th>
                ...
                {filteredRooms.map((room, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-sky-500/5 transition-colors cursor-pointer ${
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 text-sky-300 text-[11px] font-semibold inline-flex items-center gap-1.5 cursor-pointer transition-colors"
              ...
              <tfoot>
                <tr className="bg-[#141b2d] font-bold text-xs border-t-2 border-sky-500/40">
                  ...
                  <td className="py-3.5 px-4 text-right font-mono font-black text-white text-sm bg-sky-500/20 text-sky-200 border-x border-sky-500/30">
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#0a0f1d] border-b border-white/10 text-white/50 text-[11px] uppercase tracking-wider font-semibold">
                  ...
                  <th className="py-3 px-4 text-right font-bold text-rose-400">Diện tích trừ (m²)</th>
              ...
              <tbody className="divide-y divide-white/5">
                {doors.map((door) => (
                  <tr key={door.id} className="hover:bg-sky-500/5 transition-colors">
                    <td className="py-3 px-4 font-mono font-bold text-sky-300">
                      {door.code}
                    </td>
      ...
      {activeTab === 'flagged_items' && (
        <div className="bg-[#0e1424] border border-white/10 rounded-2xl overflow-hidden shadow-xl">
          <div className="px-5 py-3.5 bg-[#111728] border-b border-sky-500/20 flex items-center justify-between text-xs font-semibold">
            ...
            <span className="text-sky-200/60 text-[11px] font-mono">
              Độ tin cậy &lt; 95%
            </span>
          </div>
              ...
              <div key={room.id} className="p-5 hover:bg-white/5 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0 self-end md:self-auto text-xs">
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-sky-500/15 hover:bg-sky-500/25 text-sky-300 hover:text-white font-semibold transition-all flex items-center gap-1.5 border border-sky-500/40 cursor-pointer"
                  >
                    <span>Xem CAD</span>
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-sky-300 transition-colors cursor-pointer"
      ...
      ───────────────────────────────────────────────────────────── */}
      <div className="p-4 rounded-2xl bg-[#0c121e] border border-sky-500/30 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-sky-600 text-white flex items-center justify-center font-black font-mono text-xs shadow-sm border border-sky-400/30">
            3/4
          </div>
          <div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
          {onBackToReview && (
            <button
              onClick={onBackToReview}
              className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-sky-500/10 border border-white/10 hover:border-sky-500/30 text-xs font-semibold text-white/80 hover:text-white transition-all cursor-pointer"
            >
          ...
          {onNavigateToEstimate && (
            <button
              onClick={onNavigateToEstimate}
              className="px-5 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 active:scale-98 text-white text-xs font-bold flex items-center gap-2 shadow-[0_4px_20px_rgba(2,132,199,0.35)] border border-sky-400/30 transition-all cursor-pointer"
            >
